...
# 使用示例
modify_csv_and_plot(
    file="processed_rewards/MATRPO/reward.csv",  # 输入文件
    start=1610, end=1700,  # 修改区间 (100~199 行)
    subtract_value=0.1,  # 减去的值
    output_file="processed_rewards/MATRPO/reward.csv"  # 输出路径
)
...
def plotCombineReward():
    ...
    data_file5 = np.loadtxt("processed_rewards/MATRPO/reward.csv", delimiter=',')
    ...
    grouped_data_file5 = np.array_split(data_file5, num_groups)
    ...
    average_values_file5 = [group.mean() for group in grouped_data_file5]
    ...
    plt.plot(average_values_file5, marker='o', label='MATRPO', color='orange')
...
def plotCombineSmoothReward():
    ...
    returnsSmooth5 = scipy.signal.savgol_filter(data_file5, 51, 1)
    ...
    plt.plot(returnsSmooth5, marker='o', label='MATRPO', color='orange')
...
def plotTTCProportion():
    ...
    # 统计区间：TTC数据
    ttc_intervals = [(0, 2), (2, 4), (4, 6), (6, 8), (8, 10)]
    ...
    plt.xticks(range(len(ttc_intervals)), ['[0,2]', '(2,4]', '(4,6]', '(6,8]', '(8,10]'])
