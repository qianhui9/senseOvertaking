...
def plotResult():
    # 加载保存的CSV数据
    returns = np.loadtxt("result/ppo/reward0820.csv", delimiter=',')
...
def plotResultSmooth():
    # 加载保存的CSV数据
    returns = np.loadtxt("result/ppo/reward0820.csv", delimiter=',')
    returnsSmooth = scipy.signal.savgol_filter(returns, 15, 1)
    # 绘制折线图
...
def plotCombinedResults():
    ...
    returns_front = np.loadtxt("result/frontTraffice/reward0820.csv", delimiter=',')
...
def plotResultLCN():
    # 加载保存的CSV数据
    returns = np.loadtxt("result/ppo/numberOfLaneChanges0820.csv", delimiter=',')
...
def plotCombinedLCNResults():
    ...
    returns_front = np.loadtxt("result/frontTraffice/numberOfLaneChanges0820.csv", delimiter=',')
...
def plotLCNSec():
    ...
    data_file2 = np.loadtxt("result/frontTraffice/numberOfLaneChanges0820.csv", delimiter=',')
    ...
    plt.plot(average_values_file2, marker='o', label='frontTraffice Average', color='red')

    plt.xlabel("Group")
    plt.ylabel("LCNSec Value")
...
# LCN mean at convergence
def plotLCNSec():
    # 加载两个CSV文件的数据
    data_file1 = np.loadtxt("result/notFrontTraffice/numberOfLaneChanges0818.csv", delimiter=',')
    data_file2 = np.loadtxt("result/frontTraffice/numberOfLaneChanges0818.csv", delimiter=',')

    # 选择5000条之后的数据
    start_index = 4000
    data_file1 = data_file1[start_index:]
    data_file2 = data_file2[start_index:]
...
def plotSpeedResult():
    # 加载保存的CSV数据
    returns = np.loadtxt("result/ppo/speedControl0820.csv", delimiter=',')
...
def plotSpeedSelResult():
    returns = np.loadtxt("result/frontTraffice/speedControl0812_1.csv", delimiter=',')
    # 将每个文件的数据分成多个组，每组250个数据
    group_size = 10
...
def plotSpeedSec():
    ...
    data_file2 = np.loadtxt("result/frontTraffice/speedControl0820.csv", delimiter=',')
    ...
    plt.plot(average_values_file2, marker='o', label='frontTraffice speed', color='red')

    plt.xlabel("Group")
    plt.ylabel("SpeedSec Value")
    plt.title("Average Values per Group")
...
def plotTtcSec():
    ...
    data_file2 = np.loadtxt("result/frontTraffice/TTC0820.csv", delimiter=',')
    ...
    plt.plot(average_values_file2, marker='o', label='frontTraffice ttc', color='red')

    plt.xlabel("Group")
    plt.ylabel("TtcSec Value")
    plt.title("Average Values per Group")
...
def plotTtc():
    # 加载保存的CSV数据
    returns = np.loadtxt("result/ppo/TTC0820.csv", delimiter=',')
