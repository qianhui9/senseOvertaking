...
if __name__ == "__main__":
    ...
    for iter in tqdm(range(10000)):
        ...
        if iter == 9999:
            ...
            plt.title("Movement of UAV with Intrinsic Reward as Exploration Bonuses", fontsize=19)
            plt.xlabel("X(m)", fontsize=10)
            plt.ylabel("Y(m)", fontsize=10)
            plt.tick_params(axis='both', labelsize=9)
            plt.savefig('img/heatmap_count_based.pdf',  format='pdf')
            #plt.show()
        ...
        for j in range(10):
            ...
            for i_step in range(1000000):
                ...
                count_reward = np.zeros(env.n_bs)
                for i in range(env.n_uav):
                    ...
                    count_reward[i] = 20 / position_record[i][position_x][position_y]
                count_reward[env.n_bs-1] = 0
                ...
                for i in range(N_UAV + 1):
                    s = s_all[i]
                    a = a_all[i]
                    memorys[i].append([s, a, reward+count_reward[i], mask])
        ...
        if iter == 9000:
            ...
            checkpoint_dir = 'agent_UAV_count_based'
            if not os.path.exists(checkpoint_dir):
                os.makedirs(checkpoint_dir)
            for j in range(N_UAV + 1):
                model_path = 'agent_UAV_count_based/actor%d.pt' % j
                torch.save(actors[j].state_dict(), model_path)
                model_path = 'agent_UAV_count_based/critic%d.pt' % j
    ...
    np.savetxt('agent_UAV_count_based_1.txt', np.array(yar))
