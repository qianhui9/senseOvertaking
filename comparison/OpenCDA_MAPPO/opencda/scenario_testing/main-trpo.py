...
if __name__ == "__main__":
    ...
    for iter in range(max_episodes):  # 使用 tqdm 来显示训练进度
        ...
        current_episode_data = {i: {'speedControl': [], 'ttc': []} for i in range(N_UAV)}
        ...
        for i in range(N_UAV):
            ...
            df1.to_csv(
                f'E:/postgraduate/V2X/senseOvertaking/comparison/OpenCDA_MAPPO/opencda/scenario_testing/result/normal/agent{i + 1}_speedControl.csv',
                index=False,
                header=False)
            df2.to_csv(
                f'E:/postgraduate/V2X/senseOvertaking/comparison/OpenCDA_MAPPO/opencda/scenario_testing/result/normal/agent{i + 1}_ttc.csv',
                index=False,
                        header=False)
        ...
        # 将数据保存为CSV文件，并去除标题行
        df.to_csv('result/trpo/reward0820.csv', index=False, mode='a', header=False)
