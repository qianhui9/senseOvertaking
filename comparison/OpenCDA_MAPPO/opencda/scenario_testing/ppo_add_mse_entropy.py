...
def train_model(actor, critic, memory, actor_optim, critic_optim):
    ...
    # action_std = math.exp(-1)
    # action_size = actions[0].shape[0]
    # action_var = torch.full((action_size,), action_std*action_std)
    # action_var = action_var.expand_as(mu)
    ...
    for epoch in range(10):
        ...
        for i in range(n // hp.batch_size):
            ...
            actor_loss = -torch.min(loss, clipped_loss).mean() #+ 0.5*criterion(values, returns_samples) #- 0.01* dist_entropy
