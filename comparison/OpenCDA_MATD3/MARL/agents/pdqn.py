...
from MARL.agents.memory.memory import Memory, ReplayBuffer, NStepReplayBuffer


class QActor(nn.Module):

    # 参数解释：状态特征向量的大小，即输入层的单元数；离散或连续动作空间的大小；连续动作空间参数向量的大小，即输入层的单元数
    # 例如，如果动作由两个参数控制（例如角速度和线性速度），则action_parameter_size =2
    def __init__(self, state_size, action_size, action_parameter_size, hidden_layers=(128, 64, 32), activation="relu"):
        ...
        # create layers 一个用于保存神经网络层的列表声明
        self.layers = nn.ModuleList()
        # 输入层大小为状态特征向量的大小与动作参数向量的大小之和
        inputSize = self.state_size + self.action_parameter_size
        # 神经网络的最后一个隐藏层大小为输入层大小
        lastHiddenLayerSize = inputSize
        # 如果存在隐藏层，则在神经网络中添加它们
        if hidden_layers is not None:
            nh = len(hidden_layers)
            # 将输入层与第一个隐藏层Linear层链接，并将其添加到神经网络的层列表中
            self.layers.append(nn.Linear(inputSize, hidden_layers[0]))
            for i in range(1, nh):
                # 将当前隐藏层与前一个隐藏层Linear层链接，并将其添加到神经网络的层列表中
                self.layers.append(nn.Linear(hidden_layers[i - 1], hidden_layers[i]))
            # 最后一个隐藏层的大小为hidden_layers中的最后一个元素大小
            lastHiddenLayerSize = hidden_layers[nh - 1]
        # 和值优势层（即剩余的动作参数的处理层），大小为最后一个隐藏层的大小和动作参数向量的大小之和
    ...
    def forward(self, state, action_parameters):
        ...
        # 将状态和动作参数值串联在一起，产生一个输入向量x，dim = 1表示沿列拼接
        x = torch.cat((state, action_parameters.float()), dim=1)
        # 计算神经网络的总层数，包括输入层和所有隐藏层
        ...
        # 计算和值优势层的输出（其中每个元素对应于连续动作空间的元素），x为输入层的输出
        adv1 = self.adv_layers(x)
        # 计算值层的输出（一个常数值，代表预期回报的偏移量）
        val1 = self.val_layers(x)
        # 计算每个连续动作向量在当前状态下的优势平均值后根据Dueling DQN算法，计算Q值
...
class PDQNAgent(Agent):
    ...
    def __init__(self,
        ...
        # 每个离散动作对应 2 维连续参数
        self.action_parameter_sizes = np.array(
            [2 for _ in range(self.num_actions)])
        ...
        # 针对每个动作复制这两个参数的范围
        action_parameter_min_list = []
        action_parameter_max_list = []

        for _ in range(self.num_actions):
            action_parameter_min_list.extend([speed_min, steer_min])
            action_parameter_max_list.extend([speed_max, steer_max])

        self.action_parameter_min_numpy = np.array(action_parameter_min_list)
        self.action_parameter_max_numpy = np.array(action_parameter_max_list)
        self.action_parameter_range_numpy = self.action_parameter_max_numpy - self.action_parameter_min_numpy
