...
class LaneChangePredict():
    ...
    def __init__(self):
        ...
        self.mean_speed = 28  # 平均速度 30 m/s
        self.std_dev = 5  # 速度标准差 5 m/s
    ...
    def step(self, single_cav, action, action_param1, action_param2, i, t):
        ...
        # **新增部分：根据分布生成目标速度**
        speed = max(0, np.random.normal(self.mean_speed, self.std_dev))  # 使用正态分布生成目标速度，确保速度不为负
        yaw_rad = math.radians(yaw + th)  # 转换为弧度

        self.speed = speed

        # 计算目标速度向量
        v_x = math.cos(yaw_rad) * speed
        v_y = math.sin(yaw_rad) * speed
        ...
        reward = self.updateReward(action, speed, self.state)
