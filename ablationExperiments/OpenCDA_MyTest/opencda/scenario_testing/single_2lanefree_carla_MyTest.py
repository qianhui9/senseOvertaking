...
def runMARL(seed, episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold, replay_memory_size,
    ...
    for i_eps in range(episodes):
        ...
        avg_reward = episode_rewards / len(all_reward) # 计算平均回报

        # 更新最大值和数据
        if (i_eps >= episodes - 1) and (avg_reward > max_avg_reward):
            max_avg_reward = avg_reward
            max_episode_data = current_episode_data

        agent.end_episode()
        # 平均每步的回报
        print(i_eps, "average_episode_reward：", avg_reward)

        # 推 n-step returns
        for i in range(N_Vehicle):
            returns = compute_n_step_returns(transitions[i], gamma)
            for t, ret in zip(transitions[i], returns):
                agent.replay_memory.append(
                    state=t[0], action_with_param=t[1], reward=t[2],
                    next_state=t[3], done=t[5], n_step_return=ret
                )
        # 更新网络
        ...
        # 保存共享模型
        if save_freq > 0 == 0:
            p = os.path.join(save_dir, f"{title}.pkl")
            torch.save({
                'actor': agent.actor.state_dict(),
                'actor_param': agent.actor_param.state_dict(),
                'actor_target': agent.actor_target.state_dict(),
                'actor_param_target': agent.actor_param_target.state_dict()
            }, p)
            # print(f"Saved model at ep {i_eps + 1}: {p}")

        # 记录 reward
        pd.DataFrame({'reward': [avg_reward]}).to_csv(
            os.path.join(save_dir, 'reward.csv'),
            index=False, mode='a', header=not os.path.exists(os.path.join(save_dir, 'reward.csv'))
        )
    # 保存最终模型
    if save_freq > 0:
        final_p = os.path.join(save_dir, f"{title}_final.pkl")
        torch.save({
            'actor': agent.actor.state_dict(),
            'actor_param': agent.actor_param.state_dict(),
            'actor_target': agent.actor_target.state_dict(),
        ...
        print(f"Saved final model: {final_p}")

    # 最后，将最大平均回报轮次的数据写入文件
    for i in range(N_Vehicle):
        df1 = pd.DataFrame({'speedControl': max_episode_data[i]['speedControl']})
        df2 = pd.DataFrame({'ttc': max_episode_data[i]['ttc']})
        df1.to_csv(f'opencda/scenario_testing/result/normal/agent{i + 1}_speedControl.csv', index=False, header=False)
        df2.to_csv(f'opencda/scenario_testing/result/normal/agent{i + 1}_ttc.csv', index=False, header=False)
...
def readSenseResult(folder_id, t):
    ...
    base_path = Path("E:/postgraduate/V2X/senseOvertaking/ablationExperiments/OpenCDA_MyTest/data_dumping")
    subdirs = sorted([
