...
def load_saved_model(saved_path, model):
    ...
    def adjust_checkpoint_dims(checkpoint):
        """
        调整OrderedDict中所有以.weight结尾的权重维度顺序
        Args:
            checkpoint: OrderedDict格式的模型权重字典
        Returns:
            adjusted_checkpoint: 调整后的权重字典
        """
        adjusted_checkpoint = {}
        for key, tensor in checkpoint.items():
            if 'backbone_3d' in key and key.endswith('.weight'):
            # if key.endswith('.weight'):
                # 获取张量的维度数
                ndim = tensor.dim()

                if ndim >= 1:
                    # 生成新的维度顺序：最后一个维度移到最前，其他维度依次顺移
                    new_order = (ndim - 1,) + tuple(range(ndim - 1))
                    adjusted_tensor = tensor.permute(*new_order)
                    adjusted_checkpoint[key] = adjusted_tensor
                else:
                    # 0维或1维张量保持原样
                    adjusted_checkpoint[key] = tensor
            else:
                # 非权重参数保持原样
                adjusted_checkpoint[key] = tensor
        return adjusted_checkpoint
    ...
    if initial_epoch > 0:
        ...
        checkpoint = adjust_checkpoint_dims(checkpoint)
