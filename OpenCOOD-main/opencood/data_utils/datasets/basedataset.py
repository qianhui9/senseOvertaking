...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True):
        ...
        for (i, scenario_folder) in enumerate(scenario_folders):
            ...
            # 获取并过滤子目录名称
            dir_names = [x for x in os.listdir(scenario_folder)
                         ]
            # dir_names = [x for x in os.listdir(scenario_folder)
            #              if os.path.isdir(os.path.join(scenario_folder, x))]

            # 按数字后缀排序（假设目录名为 cav_1, cav_2, ...）
            # cav_list = sorted(dir_names, key=lambda x: int(x.split('_')[1]))
            ...
            for (j, cav_id) in enumerate(cav_list):
                ...
                for timestamp in timestamps:
                    ...
                    yaml_file = os.path.join(timestamp + '.yaml')
                    lidar_file = os.path.join(timestamp + '.pcd')
                    # camera_files = self.load_camera_files(cav_path, timestamp)
