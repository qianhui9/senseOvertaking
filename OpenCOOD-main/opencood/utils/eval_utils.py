...
def calculate_mota_per_iou(result_stat):
    """
    Calculate the MOTA score for each IoU threshold (0.3, 0.5, 0.7)

    result_stat: Dictionary containing TP, FP, GT, and ID Switches at various IoU thresholds
    """
    mota_values = {}

    # Iterate over each IoU threshold (0.3, 0.5, 0.7)
    for threshold in result_stat:
        tp = result_stat[threshold]['tp']
        fp = result_stat[threshold]['fp']
        gt = result_stat[threshold]['gt']
        idsw = result_stat[threshold].get('idsw', 0)  # ID Switches, default to 0 if not provided

        total_tp = sum(tp)
        total_fp = sum(fp)
        total_fn = gt - total_tp  # FN = GT - TP
        total_idsw = idsw
        total_gt = gt

        # Calculate MOTA for this threshold
        mota = 1 - (total_fp + total_fn + total_idsw) / total_gt if total_gt > 0 else 0
        mota_values[threshold] = mota
